...
public class UserServise {
    ...
    public ResponseEntity<Void> rigisterUser(UserDto body) {
        try {
            ...
            UsersModel build = UsersModel.builder().login(body.getLogin()).password(encoder.encode(body.getPassword()))
                    .role(role).name(body.getName()).surname(body.getSurname()).patronymic(body.getPatronymic()).email(body.getEmail()).build();
    ...
    public ResponseEntity<Map<Object, Object>> login(UserDto body, HttpServletRequest request) {
        Optional<UsersModel> user = userRepository.findByLogin(body.getLogin());
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder(salt, new SecureRandom(new byte[20]));
